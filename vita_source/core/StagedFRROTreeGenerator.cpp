...
int StagedFRROTreeGenerator::isValidRootSegment(point xNew,
	...
	if( !(domain->isSegmentInside(tree->getXProx(),xNew)) )
	  {
	    cout << "Root not inside the domain." << endl;
	    return false;
	  }
	
	point dVect = xNew - tree->getXProx();
	cout << "Is root segment satisfying the dLim constraint? " << (sqrt(dVect ^ dVect) > dLim) << endl;
...
#pragma omp critical
			...
			// point n = (tree->getRoot()->getVessels()[0]->xDist)%(tree->getRoot()->getVessels()[0]->xProx);
			point xprox = tree->getRoot()->getVessels()[0]->xProx;
			point xdist = tree->getRoot()->getVessels()[0]->xDist;
			point n = xprox%xdist; // Normal to the plane
			double a1 = n^xNew, a2 = n^minBif; // Which side of the plane do they lie in (superior/inferior quadrant)
			// double a3 = xdist^xNew, a4 = xdist^minBif; // Which side of the plane do they lie in (temporal/nasal)
			
			// if ((minCost < INFINITY) && ((a1*a2)>0) &&((a3*a4)>0)) {
			if ((minCost < INFINITY) && ((a1*a2)>0)) {
